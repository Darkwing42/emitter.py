...
def test_on_1():
    """
    on() called without params returns a set containing all registered events.
    """
    emitter = Emitter()
    ...
    # checking value
    assert emitter.on() == {"event1", "event2"}


def test_on_2():
    """
    on() called without params on an empty emitter returns an empty set.
    """
    emitter = Emitter()
    ...
    # checking value
    assert emitter.on() == set()


def test_on_3():
    """
    on() called with only the event param returns an OrderedDict.
    It contains all the listeners for the event, along with listener's credit.
    """
    emitter = Emitter()
    emitter.on("event", callable)
    emitter.on("event", bool, 10)

    # checking type
    assert isinstance(emitter.on("event"), OrderedDict)

    # checking value
    event_items = list(emitter.on("event").items())
    assert event_items == [(callable, -1), (bool, 10)]


def test_on_4():
    """
    on() called with only the event param returns an OrderedDict.
    Even if no listeners registered for that event.
    """
    emitter = Emitter()
    ...
    # checking value
    assert emitter.on("nonexistent") == OrderedDict()
